...
parser.add_argument('--epochs', default=700, type=int, metavar='N',
                    help='number of total epochs to run (default: 700')
...
def main():
    ...
    if args.seed == 0:
        ...
        setup_my_seed(seed=args.seed)  # 固定种子
...
def train(train_loader, model, criterion, optimizer, epoch, SumWriter, args, KD_Label_bank):
    ...
    # 创建 Son_Label_bank 以收集每个epoch产生标签的sum，每个epoch迭代初始化为0
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        L3_4_Binaryout, L3_4_Realout, L3_5_Binaryout, L3_5_Realout, L3_6_Binaryout, L3_6_Realout = model(input)
        ...
        L1_5_loss = Feature_loss(L1_5_Binaryout, L1_5_Realout.detach())
        L1_6_loss = Feature_loss(L1_6_Binaryout, L1_6_Realout.detach())
        ...
        L2_5_loss = Feature_loss(L2_5_Binaryout, L2_5_Realout.detach())
        L2_6_loss = Feature_loss(L2_6_Binaryout, L2_6_Realout.detach())
        ...
        L3_5_loss = Feature_loss(L3_5_Binaryout, L3_5_Realout.detach())
        L3_6_loss = Feature_loss(L3_6_Binaryout, L3_6_Realout.detach())

        # 中间损失，一共具有18个卷积层，所以18个对应损失
        Fmap_loss = L1_1_loss + L1_2_loss + L1_3_loss + L1_4_loss + L1_5_loss + L1_6_loss + \
                    L2_1_loss + L2_2_loss + L2_3_loss + L2_4_loss + L2_5_loss + L2_6_loss + \
                    L3_1_loss + L3_2_loss + L3_3_loss + L3_4_loss + L3_5_loss + L3_6_loss
        Fmap_loss = args.Feature_alpha * Fmap_loss / 18.0
    ...
    if (epoch + 1) % 100 == 0:
        ...
        # 添加 本次epoch的 最后一个 mini-batch的 第一张图片 对应的 所有中间特征图
        # 若干图合为一个
        ...
        L1_6_Binaryout = vutils.make_grid(L1_6_Binaryout[2].float().detach().cpu().unsqueeze(dim=1), nrow=32, normalize=True)
        SumWriter.add_image("L1_6_Binaryout", L1_6_Binaryout, epoch)
        L1_6_Realout = vutils.make_grid(L1_6_Realout[2].float().detach().cpu().unsqueeze(dim=1), nrow=32, normalize=True)
