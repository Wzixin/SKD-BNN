...
class BinarizeLinear(nn.Linear):
    ...
    def forward(self, input, realinput):
        ...
        with torch.no_grad():
            ...
            Real_output = nn.functional.linear(Ra, Rw)
            Real_output = torch.abs(out) * torch.sign(Real_output)

        if not self.bias is None:
            self.bias.org=self.bias.data.clone()
            out += self.bias.view(1, -1).expand_as(out)
            Real_output += self.bias.view(1, -1).expand_as(Real_output)  # ++

        return out, Real_output
